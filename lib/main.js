...
import scrollIntoView from "dom-scroll-into-view/dist-src";
  ...
  renderItems() {
    ...
    const $empty =
      items && items.length ? (
        ...
        <li className="autocomplete-li empty">{this.props.emptyMessage}</li>
      );
    return !isOpen ? (
      undefined
    ) : (
      <ul className="results" onMouseOver={this.onMouseOver}>
        {$empty ||
